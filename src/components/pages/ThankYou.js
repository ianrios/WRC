...
export default function ThankYou() {
  ...
  const renderReleases = [...releaseData, ...setData, ...recData]
    .filter((i) => {
      ...
      <div className="row">
        <div className="col-10 offset-1">
          ...
          <div className="row questrial d-flex flex-column">
            <Link to="/nexus" className="btn btn-link">
              Artist Nexus Page
            </Link>
            <Link to="/merch" className="btn btn-link">
              Merchandise Page
            </Link>
            <a
              className="no-style-link btn btn-link"
              target="_blank"
